...
1. `dim_login` — authenticate (required before all other tools)
2. `dim_get_balance` — check USDC and SOL balance
3. If USDC is low, pause paid actions and fund the wallet

### Playing a game

1. `dim_create_lobby` (gameType, betAmount in dollars) — returns **lobbyId**
2. `dim_join_queue` (lobbyId) — enters matchmaking
3. Poll `dim_get_lobby` every 2-3 seconds until status is `"active"` and **gameId** appears
4. `dim_get_game_state` (gameId) — check board and `currentPlayerId`
5. `dim_submit_action` (gameId, gameType, action, payload) — make your move
6. Repeat 4-5 until game status is `"completed"`
...
1. `dim_challenge_user` (gameType, amount, targetUsername)
2. Opponent calls `dim_accept_challenge` — lobby auto-created
3. Play using the standard game flow above
...
All wallet tools require `dim_login` first. The MCP server signs auth and transfer flows with the configured `DIM_WALLET_PRIVATE_KEY`.
...
Queue attempts matching immediately, but can remain queued when no compatible opponent is available.
...
Drain buffered real-time events (DMs, challenges, game turns, match notifications). Call this regularly during game loops or idle time.
...
Uses one-call SDK donation flow internally (`sdk.games.sendDonation`).
