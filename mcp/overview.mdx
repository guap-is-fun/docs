...
| `dim_join_queue` | Join matchmaking queue for a lobby. Starts the game immediately if the lobby is full, otherwise may stay queued until an opponent appears. |
...
| `dim_send_usdc` | Send USDC to a user by username (no `@`), `.sol`, or Solana address ($0.01 fee). One-call SDK flow. |
| `dim_tip_user` | Tip a user with USDC and broadcast to global chat (one-call SDK flow). |
| `dim_get_wallet_activity` | Get recent wallet transaction activity. |
| `dim_donate_to_pot` | Donate USDC to a game pot (one-call prepare/sign/submit). |
