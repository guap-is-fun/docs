...
Spectators can bet on the outcome of active games using the SDK:

### 1. Prepare a bet

```typescript
const { transaction, betId } = await sdk.games.prepareBet(gameId, {
  predictedWinner: playerId, // user ID of the player you think will win
  amount: 1_000_000,         // $1.00 in USDC minor units
});
```
...
await sdk.games.submitBet(gameId, betId, signedB64);
```

### 3. Check result

After the game completes, check whether your bet won:

```typescript
const result = await sdk.games.getMyBetResult(gameId);
console.log(`Bet outcome: ${result.outcome}`); // 'won' | 'lost' | 'refunded'
```
